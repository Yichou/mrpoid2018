...
public abstract class BaseMrpListFragment extends BaseFileFragment {
	...
	public BaseMrpListFragment(int loaderId) {
		super(loaderId);
	}
...
//			MrpRunner.runMrp(getActivity(), file.getPath());
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
		if(v == getListView()) {
			...
			if(file.isFile()) {
				menu.add(mLoaderId, R.id.mi_run_mode, 0, R.string.run_mode);
				menu.add(mLoaderId, R.id.mi_remove, 0, R.string.remove);
				menu.add(mLoaderId, R.id.mi_add_favorite, 0, R.string.add_favorite);
				menu.add(mLoaderId, R.id.mi_create_shortcut, 0, R.string.create_shortcut);
			}
		} else {
	...
	public boolean onContextItemSelected(android.view.MenuItem item) {
		...
		if (item.getItemId() == R.id.mi_remove) {
			...
			HomeActivity.showRunMrpModeDialogFragment((ActionBarActivity)getActivity(), file.getPath());
		} else 
		{
			return super.onContextItemSelected(item);
		}
